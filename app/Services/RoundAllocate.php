...
class RoundAllocate
{
     ...
     */
    public function songs(Stage      $stage,
                          Collection $songs,
                          ?int       $songs_per_round = null,
                          ?Carbon    $round_start = null,
                          string     $round_duration = '7 days'): void
    {
        if ($songs->isEmpty())
        {
        ...
        // Split Songs into Rounds.
        if ($songs_per_round > 1)
        {
            $songs       = $songs->shuffle()->shuffle();
